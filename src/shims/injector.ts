...
import { InjectFlags, inject } from './injector-compatability'
import { Inject, Optional, Self, SkipSelf } from 'ng-metadata/core'
import {
  ConstructorProvider,
  ExistingProvider,
  FactoryProvider,
...
function formatError(text: string, obj: any, source: string | null = null): string {
  ...
  return `StaticInjectorError${source ? '(' + source + ')' : ''}[${context}]: ${text.replace(
    NEW_LINE,
    '\n  '
  )}`
}
